...
# Plot
x |> 
  ggplot(aes(x = kind, y = g4math, fill = race)) +
...
question : Which combination of student characteristics (e.g., class size, race, years in small class, school) best predicts future academic success in math?

Preceptor table:

1. predictive model
2. units: students
3. outcome: grade/test_scores
4. covariates:race/yearssmall
5. treatments : class size/ kind
...
1. validity: how students class size acurately reflects the maths school ?
2.stability: Would the effect of class size on math performance remain stable across different contexts or over time?
3.Are the students in the STAR study representative of the population we want to generalize to?
(The results from the Tennessee STAR dataset may not be fully generalizable to a Chicago school context because of differences in student demographics, socioeconomic conditions, urban school environments, and possibly curriculum standards.)
4.Unconfoundedness means that, apart from the treatment (class size), there are no unmeasured variables that simultaneously affect both treatment assignment and the outcome (math performance).
$$
